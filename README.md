...
## 🧩 Basic Concept — RAG-Based Company AI Chatbot

The system is being developed as our own company-specific AI chatbot, designed to answer questions using our company's own knowledge and selected external information sources.

### 📂 1. Company Data & Documents

First, we collect company-related information such as:
...
- Product information
- Technical details
- Company procedures and other relevant data

Large documents are processed using a technique called **Chunking**, where the information is divided into smaller, meaningful sections.

These sections are then converted into **Vector Embeddings** and stored in a specialized **Vector Database**.

This forms the basic foundation of a **RAG (Retrieval-Augmented Generation)** system.

### 🌐 2. Website Crawling

The system can also be connected to selected websites.

It can crawl those websites, collect relevant information, process and chunk the content, convert it into vector embeddings, and store the information in the knowledge database.

Therefore, the chatbot's knowledge base can contain information from:

- 📁 Our Company's Internal Data
- 🌐 Selected External Websites

### 🔍 3. How the Chatbot Works

When a user asks a question, the chatbot does not simply depend on the AI's general knowledge.

The process is approximately:

```text
User Question → Search Knowledge Base → Retrieve Relevant Information → Send Context to Gemini → Generate Answer
```

The system searches the Vector Database and identifies the most relevant information related to the question. That information is then provided to Gemini, which uses the retrieved context to generate a natural-language response.

### 💡 4. In Simple Terms

The goal is to build our own AI-powered company knowledge assistant.

The chatbot will be able to search our internal company information as well as information collected from selected websites, and then provide an answer based on the relevant retrieved data.

### ⚠️ Current Development Status

It is important to note that the complete company document set has **NOT yet** been uploaded into the RAG system.

The system is currently under development/testing, and only the available/selected data has been processed and added to the knowledge base so far.

As more company documents and relevant information are processed and added, the chatbot's company-specific knowledge base will become more comprehensive.

**In short:**

```text
Our Data + Selected Web Data → Chunking → Vectorization → Vector Database → RAG Retrieval → Gemini → Company-Specific AI Answer 🤖
```
